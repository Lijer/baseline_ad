...
from utilities import dataLoading, cutoff_unsorted, aucPerformance, writeRepresentation,writeResults, writeOutlierScores
import time
...
#    writeRepresentation(new_X, labels, embedding_size, filename + str(embedding_size) + "D_RankOD")
    scores = lesinn(new_X)
    rauc,_ = aucPerformance(scores, labels)
...
for i in range(30):
    ...
    rauc,ap = aucPerformance(outlier_scores, labels)
    ROCAUC_list.append(rauc)
    ap_list.append(ap)
